...
export default class RepresentantesPrecos2 extends Component {
  state = {
    titulo: "",
    file: "",
    loader: false,
  ...
  static navigationOptions = ({ navigation }) => ({
    title: "Tabela de Preços 2"
  });

  loadNoticias = async (page = 1) => {
    const response = await api.get(`tabela_precos2.php?page=${page}`);
    ...
    const infoUrl = this.state.noticias.map(data => {
      this.setState({ url: `https://www.zummo.com.br/_app/uploads/tabela_precos2/${data.file_name}` });
    });
  };
