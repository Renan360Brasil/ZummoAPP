...
export default class RepresentantesTamanhos extends Component {
  state = {
    titulo: "",
    file: "",
    loader: false,
  ...
  static navigationOptions = ({ navigation }) => ({
    title: "Medidas das Máquinas"
  });

  loadNoticias = async (page = 1) => {
    const response = await api.get(`tabela_tamanhos.php?page=${page}`);
    ...
    const infoUrl = this.state.noticias.map(data => {
      this.setState({ url: `https://www.zummo.com.br/_app/uploads/tabela_tamanhos/${data.file_name}` });
    });
  };
