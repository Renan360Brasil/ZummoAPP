...
export default class RepresentantesDescontos extends Component {
  state = {
    titulo: "",
    file: "",
    loader: false,
  ...
  loadNoticias = async (page = 1) => {
    const response = await api.get(`tabela_descontos.php?page=${page}`);
    ...
    const infoUrl = this.state.noticias.map(data => {
      this.setState({ url: `https://www.zummo.com.br/_app/uploads/tabela_descontos/${data.file_name}` });
    });
  };
